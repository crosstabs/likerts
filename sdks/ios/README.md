...
In Xcode, choose **Add Package Dependencies → Add Local** and select `/absolute/path/to/likerts/sdks/ios`, then link its `Likerts` product to your app target. For a SwiftPM consumer, use `.package(name: "Likerts", path: "/absolute/path/to/likerts/sdks/ios")` and `.product(name: "Likerts", package: "Likerts")`. The package uses Swift tools 5.9 and supports iOS 15+ and macOS 12+. The repository root now also exposes the same `Likerts` product through a Swift package manifest. A new public semantic-version tag and fresh remote consumer check are still pending; use the local package path until [native distribution evidence](../../docs/verification/native-distribution.md) records publication. No binary framework is claimed.
