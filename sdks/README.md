...
The collection-only bearer token authorizes GET `/v1/collections/{id}` and POST `/v1/collections/{id}/responses`. Configurations use immutable schema versions 1–5 and support `single_choice`, `multiple_choice`, `scale`, `text`, `number`, `date`, `ranking`, `matrix` and `constant_sum`. Metadata supplied by the app is untrusted. Swift uses Codable/Sendable `JSONValue` for nested metadata objects, arrays, numbers, booleans and null; all clients accept JSON metadata objects. Swift `submit` returns a typed `Receipt` and rejects an unaccepted or incorrectly charged receipt.
