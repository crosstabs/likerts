...
Select Render for the Rust API and separate callback worker, Neon Postgres through the Vercel Marketplace for primary data, and the separately configured private Vercel Blob adapter for exports. Use Singapore as the initial staging and launch region default, with region recorded per workspace. This is not a universal residency promise. Model provider costs before provisioning production capacity.

Use one shared deployment with database-enforced tenant boundaries. Before public launch use at least two API instances and qualify Neon's selected plan, restore window and failure behavior in staging. Migrations run as a separate manual one-off job based on an inert cron service with owner-only credentials; API/worker never receive the owner DSN. Runtime roles are created with SQL so Neon does not add `neon_superuser`; they cannot own tables, inherit memberships, bypass RLS or alter schema. RLS denies missing tenant context and transaction-local settings cannot leak through the pool. Public collection access gets a separately bounded authorization path; never grant unrestricted database access to an SDK capability.
