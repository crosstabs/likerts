...
`LIKERTS_RECEIVER_REDIS_TEST=1 node --test infrastructure/acceptance/receiver/test/receiver.test.mjs` passed all 12 checks locally on Node 22.22.2, including the actual eight-second timer and a real isolated Redis test: the marker was readable while the first reply was held, 20 concurrent duplicates returned 204 before that reply, and capacity/TTL checks passed. Required CI now runs this command after `scripts/check-admission.sh`, whose successful Redis container startup makes `redis:7-alpine` available for the receiver test's `--pull=never`. Local preparation does not establish remote CI success until that revision passes its required checks.

Remaining bounded work: deploy and verify the isolated receiver; establish reviewed isolated API/worker process topology and account SSH access; execute exact-instance loss with scoped polling; preserve timestamped aggregate evidence; restore resources. The observed API PID 1 requires a separately reviewed and tested init wrapper or verified provider termination approach. DNS across-attempt validation needs the owned hostname/TLS and leaf-only change; deterministic hosted rebinding needs additional authoritative-DNS control. None of these steps implies network-enforced egress isolation or a failover SLA.
