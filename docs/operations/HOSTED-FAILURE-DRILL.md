...
Reviewed 2026-09-10; isolated receiver preparation verified locally 2026-09-13. This document is a bounded operator procedure, not hosted interruption evidence. No service, DNS record, SSH key or provider setting was changed during this local preparation. The image supports Render's SSH prerequisites, the export verifier checks a new lease, and the signed receiver now supports a verified-marker hold; account access and exact-instance interruption still need execution.
...
For the next isolated run, the [disposable supervisor fixture](../../infrastructure/failure-fixture/README.md) avoids the observed production PID 1 constraint without changing production. It copies the released API/worker binaries into a separate container and permits one authenticated, identity-checked kill per child. Local native guard tests and a constrained emulated lifecycle passed; peak web memory was 180.1 MiB under a 512 MiB/no-swap limit with PostgreSQL separate. A later [native hosted export drill](../verification/hosted-export-recovery.md) created separate Free Render web/PostgreSQL resources after allowance/plan checks and confirmed exact API child loss while a committed lease was running. Its same-job recovery after the actual 300-second expiry, different winning fence and verified one-response download passed. Fixture collections/credentials were revoked and the owned web/database resources independently returned 404 after deletion. Those provider operations are separate from the earlier local check. The SSH procedure below remains an alternative for an independently authorized, eligible instance.
...
Remaining bounded work: configure a fresh isolated receiver for an actual worker endpoint and execute expired worker-lease recovery; rehearse compatible rollback and preserve timestamped evidence. The reviewed disposable supervisor provides a separate native child-process path; account SSH remains a prerequisite only for the alternative SSH procedure. The observed production API PID 1 was not signaled. DNS across-attempt validation needs the owned hostname/TLS and leaf-only change; deterministic hosted rebinding needs additional authoritative-DNS control. None of these steps implies network-enforced egress isolation or a failover SLA.
