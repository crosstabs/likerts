...
Reviewed 2026-09-10. This document is a bounded operator procedure, not execution evidence. No service, DNS record, SSH key or provider setting was changed during this review. Local changes enable Render's SSH prerequisites and fix the export verifier; hosted access and interruption still need execution.
...
Smallest additional fixture change: in a separate isolated receiver deployment, add an environment-only **hold-first-verified-receipt for eight seconds** mode. Record the authenticated event marker before holding, and hold only the first atomic insertion. Retries must return 204 immediately. Do not add a public sleep parameter or change the ordinary receiver's defaults. Keep existing HMAC/schema checks, 2,000-record cap and one-hour TTL.
...
2. Use one synthetic accepted response with a disabled-then-enabled fixture endpoint (or a single planned synthetic threshold event), and observe its verified receiver marker plus database `running` attempt. Record lease expiry privately.
