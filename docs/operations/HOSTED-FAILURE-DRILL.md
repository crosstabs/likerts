...
Remaining bounded work: deploy and verify the reviewed API/worker image and isolated receiver; verify account SSH and live process topology; execute exact-instance loss with scoped polling; preserve timestamped aggregate evidence; restore resources. If PID1 blocks direct loss, agree and test an init-wrapper change separately. DNS across-attempt validation needs the owned hostname/TLS and leaf-only change; deterministic hosted rebinding needs additional authoritative-DNS control. None of these steps implies network-enforced egress isolation or a failover SLA.
