...
Production operational telemetry uses same-origin endpoints on the existing Vercel deployment. `/api/client-events` accepts only a strict browser-error fingerprint contract; `/api/product-events` accepts only the existing coarse product-event allowlist. Those automatic browser requests omit credentials and contain no persistent user/session identifier, raw URL, message, stack, prompt, research material, or application state. Both endpoints apply bounded bodies, same-origin checks, strict schemas, per-client abuse limits, and structured server-side redaction before writing ordinary Function logs. No additional telemetry provider or dynamically injected browser runtime is involved. See the [monitoring and product-improvement runbook](monitoring-and-product-improvement.md).

`/api/feedback` is deliberately separate because it accepts explicit user-provided text. The side panel tells visitors not to include personal information or research material and discloses that the message, category, page path, and interface language are written to operational logs. The contract accepts no account, email, query string, fragment, cookie, or persistent visitor ID; it bounds messages to 800 characters and applies same-origin checks, strict validation, and a low submission rate limit. Feedback inherits the configured Vercel Runtime Log retention and access controls, so longer retention or forwarding requires the same documented approval as any other log drain.
