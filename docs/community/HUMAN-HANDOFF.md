...
| Search Console | Existing verified `sc-domain:likerts.com` access is available. | Existing sitemap receipt and current URL inspection results are recorded in [Search Console evidence](../verification/search-console.md). No new login is required; indexing remains Google’s decision. |
| Claude end-to-end agent call | Run `claude auth login --claudeai` and complete the normal subscription login. | Codex now passes a real tool call. After Claude login, run only `--clients=claude`; see [client evidence](../verification/agent-clients.md). |
...
| --- | --- | --- |
| Codex CLI `0.154.0` | Corrected harness passed: initialize, discover 33 tools, `surveys_list` and exact empty result through public MCP `0.1.0`; Astra low, exit 0 in 15.649 seconds. | Earlier probes waited on open stdin; timeout handling also mistook a graceful zero exit for ordinary completion. Both harness bugs are fixed and regression-tested. [Actual evidence](../verification/agent-clients.md). |
...
Google documents [Search Console setup and URL Inspection](https://developers.google.com/search/docs/monitor-debug/search-console-start) and [sitemap submission](https://developers.google.com/search/docs/crawling-indexing/sitemaps/build-sitemap). The public robots file already advertises the sitemap. The existing verified property has now been inspected; its sitemap receipt predates this session. Current page results are recorded separately without claiming a new submission or guaranteed indexing.

The original finite OSS milestone completed preparation. The expanded finish-all goal remains active while executable work continues and these dependencies remain unresolved. Real authorship, login, support commitments and external adoption cannot be replaced by waiting, test traffic or an AI pretending to be a person.
