...
The evidence contains no credentials, collection IDs, response payloads or private file paths. [The checked-in rehearsal result](newcomer-evidence.json) preserves the earlier actual local run with `community-v0.1.0`; it is not rewritten as a new run. The current command selects `community-v0.1.2`, whose changed default is checked by required CI. It establishes released-client compatibility with the checked-out backend; it does not establish hosted sign-in, production capacity, managed recovery or an availability commitment.
