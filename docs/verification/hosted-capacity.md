...
Checked 2026-09-13: provider API inspection at 01:50 UTC, followed by the existing Vercel-to-Upstash console session at approximately 02:08 UTC. The authenticated Upstash database view was rechecked on 15 September. These are read-only observations of the existing services, not a load test or a capacity guarantee. No plan, region, configuration, deployment, account access, or resource was changed.
...
| 15 September plan readback | Database `likerts-production-admission`: Free Tier, AWS N. Virginia (`us-east-1`), Global, 10,000 commands/second, 10 MB request, 100 MB record, 256 MB data and 50 GB bandwidth. The dashboard showed 12k / 500k monthly commands, 1 KB / 256 MB storage, 0 B / 50 GB bandwidth and $0.00 cost. | Confirms the current console plan and ceilings. Rounded/interval displays still do not establish an exact workflow billing multiplier or an alert path. |
...
The operational decision is to keep the managed service described as a bounded reference preview and to withhold unrestricted hosted traffic claims. The current Free database is adequate for that disclosed preview only while its low-volume usage stays observed; it is not approved as a general hosted capacity allocation. Expanding the claim requires a proved exhaustion/missing-run alert path and human responders under H04, followed by a controlled billing-boundary measurement or an explicit paid capacity decision.

Hosted admission capacity therefore remains unverified for unrestricted traffic. The bounded network and API timing evidence above is complete for its stated scope. Health success and local multi-process limiter tests remain useful evidence of different properties.
