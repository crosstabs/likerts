...
| Android | Central-ready POM fields, source JAR, generated Dokka HTML inside the `javadoc` JAR, optional in-memory signing. | Unsigned local Maven staging succeeded with AAR/POM/module/source/docs artifacts and checksums; docs JAR has 300 entries including index and MIT license. Required POM sections are present. | Namespace verification is complete. Central publisher token, owner release-signing identity/email and key/public-key distribution, signed bundle validation/upload, publication and fresh Maven Central consumption remain required. |
...
The namespace is now verified; the owner has been asked to supply the public release-signing email/identity. This is separate from the publisher token and the signing key, neither of which is established by the namespace status.
