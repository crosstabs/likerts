...
const port = Number(process.env.LIKERTS_EXAMPLE_PORT ?? 4350);
const apiPort = Number(process.env.LIKERTS_EXAMPLE_API_PORT ?? 4351);
...
const scratch = await mkdtemp(join(tmpdir(), 'likerts-vue-'));
const token = randomBytes(32).toString('hex');
const workspace = `vue-${randomUUID()}`;
const children = [];
let stopping = false;
