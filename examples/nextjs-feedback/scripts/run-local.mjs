...
const port = Number(process.env.LIKERTS_EXAMPLE_PORT ?? 4330);
const apiPort = Number(process.env.LIKERTS_EXAMPLE_API_PORT ?? 4331);
...
const scratch = await mkdtemp(join(tmpdir(), 'likerts-nextjs-'));
const token = randomBytes(32).toString('hex');
const workspace = `nextjs-${randomUUID()}`;
const children = [];
let stopping = false;
...
try {
  ...
  const env = { ...minimal, NEXT_TELEMETRY_DISABLED: '1', LIKERTS_EXAMPLE_LOCAL_OPERATOR: '1', LIKERTS_EXAMPLE_ORIGIN: origin, LIKERTS_API_URL: apiOrigin,
    LIKERTS_TOKEN: token, LIKERTS_COLLECTION_ID: collection.id, LIKERTS_COLLECTION_TOKEN: collection.token };
  const args = [join(here, 'node_modules/next/dist/bin/next'), 'start', '--hostname', '127.0.0.1', '--port', String(port)];
  if (check) {
  ...
  const next = spawn(process.execPath, args, { cwd: here, env, stdio: ['ignore', 'pipe', 'pipe'] });
  children.push(next);
  await waitFor(origin, next);
  console.log(`Next.js feedback example: ${origin}\nReal local API, collection-scoped browser credential; temporary data clears on shutdown.`);
  if (check) {
  ...
  for (const child of [api, next]) child.once('exit', () => { if (!stopping) stop(1); });
