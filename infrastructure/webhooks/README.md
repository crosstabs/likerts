...
This is customer-configured response-event delivery. Likerts does not send invitations, distribute surveys, or create respondent links. The receiver URL belongs to the customer.

Seven management operations, API/MCP/CLI parity, the durable worker and migration `backend/migrations/0017_response_webhooks.sql` are implemented. The PostgreSQL lifecycle/privilege/interface gate and CDK synthesis assertions pass locally. Hosted HTTPS transport and AWS deployment evidence remain separate release gates. See `contracts/CAPABILITIES.md` for all operation inputs, scopes, outputs and errors.
...
- Event/delivery/attempt records expire seven days after response acceptance. Response erasure and workspace deletion delete their queued callback records transactionally. A request already sent cannot be recalled.
...
Run `bash scripts/check-webhook-isolation.sh`. It creates and removes an isolated PostgreSQL 17 container, applies all current migrations including 0017, provisions actual restricted API/worker logins and tests:
...
- All seven real API/MCP/CLI operations against restricted PostgreSQL, scope/cross-tenant failures, eight retries producing one event/one cent, and worker startup refusing the API database identity.
