...
export const FIRST_RECEIPT_HOLD_MS = 8000;
...
export function loadConfig(env = process.env) {
  const holdFirst = env.LIKERTS_ACCEPTANCE_HOLD_FIRST_RECEIPT ?? '0';
  requireValue(['0', '1'].includes(holdFirst), 'receiver_not_configured');
  ...
  return { holdFirstReceipt: holdFirst === '1', namespace, adminToken, keys, redisUrl: url.origin, redisToken, redisKey: `likerts:acceptance:${namespace}:events` };
}
...
export function verifyEvent(raw, headers, keys, nowMs = Date.now()) {
  ...
  requireValue(exactKeys(body, ['id', 'type', 'eventVersion', 'createdAt', 'data']) && body.id === eventId
    && body.type === 'response.accepted' && body.eventVersion === 1
  ...
  requireValue(exactKeys(data, ['responseId', 'collectionId', 'surveyId', 'surveyVersion'])
    && [data.responseId, data.collectionId, data.surveyId].every(value => UUID.test(value ?? ''))
    && Number.isInteger(data.surveyVersion) && data.surveyVersion >= 1 && data.surveyVersion <= 2147483647, 'invalid_event', 400);
  return { eventId, deliveryId, attemptId, eventType: body.type, signatureVerified: true,
    responseId: data.responseId, collectionId: data.collectionId, surveyId: data.surveyId, surveyVersion: data.surveyVersion };
}
...
export async function receive(request, dependencies = {}) {
  ...
  try {
    ...
    // Redis committed the verified marker before this test-only hold. A concurrent
    // duplicate gets result[0] === 0 and immediately acknowledges the same event.
    if (config.holdFirstReceipt && result[0] === 1) {
      await (dependencies.sleep ?? (ms => new Promise(resolve => setTimeout(resolve, ms))))(FIRST_RECEIPT_HOLD_MS);
    }
...
function storedEvent(value) {
  ...
  requireValue(exactKeys(event, ['eventId', 'deliveryId', 'attemptId', 'eventType', 'signatureVerified', 'responseId', 'collectionId', 'surveyId', 'surveyVersion'])
    && UUID.test(event.eventId ?? '') && UUID.test(event.deliveryId ?? '') && UUID.test(event.attemptId ?? '')
    && event.eventType === 'response.accepted' && event.signatureVerified === true
    && [event.responseId, event.collectionId, event.surveyId].every(value => UUID.test(value ?? ''))
    && Number.isInteger(event.surveyVersion) && event.surveyVersion >= 1 && event.surveyVersion <= 2147483647, 'storage_unavailable');
  return event;
}
