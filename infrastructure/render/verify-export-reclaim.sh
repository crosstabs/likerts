...
select status || '|' || coalesce(lease_id::text,'') || '|' ||
  case when object_key is not null and lease_id is not null
    and object_key = id::text || '.' || lease_id::text || '.export' then 't' else 'f' end
