...
for(const [name,mode,env,expected] of [
  ...
  ['worker rejects admission token','worker',{LIKERTS_ADMISSION_REST_TOKEN:sentinel},'Non-worker credential forbidden'],
  ['MCP rejects admission token','mcp',{LIKERTS_ADMISSION_REST_TOKEN:sentinel},'Data-plane credentials forbidden'],
  ['API rejects disabled admission','api',{DATABASE_URL:'synthetic',LIKERTS_ADMISSION_MODE:'disabled'},'Distributed admission is mandatory'],
  ['API requires admission configuration','api',{DATABASE_URL:'synthetic'},'Admission Redis origin required'],
