...
sql(`insert into likerts.webhook_endpoints(workspace_id,id,url,enabled,key_id,key_hash) values ('container-smoke','${webhookEndpoint}','https://hooks.customer.com/recovery',true,'${randomUUID()}',decode(repeat('01',32),'hex'))`);
...
try{
  ...
  const evidence={measuredAt:new Date().toISOString(),imageId:docker(['image','inspect',image,'--format','{{.Id}}']),postgresVersion:sql('show server_version'),backupBytes,backupMs:Math.round(backupMs),restoreAndVerificationMs:Math.round(recoveryMs),journalEvents,restoredAcceptedResponses:2,visibleResponsesAfterReplay:1,ledgerCents:2,creditEntriesPreserved:creditEntries,promotionalCreditsRemaining:998,forcedRlsTables:Number(permissions),appliedMigrations:Number(versions),checks:['logical backup/restore','post-backup deletion journal replay','old service/collection capabilities denied','restored exports revoked','restored callback endpoints and queues quarantined','worker customer-data denial preserved','retained answer readable after new credential issuance','unchanged one-cent ledger','unchanged promotional grant and consumption ledger','onboarding grant remains idempotent after restore','runtime credit mutation and worker credit read denied','forced RLS preserved','failed transactional DDL rolls back','migration checksum mismatch blocks promotion','correct checksum reruns safely'],limitations:['Tiny local logical-dump fixture; not a managed snapshot/PITR or production RTO/RPO result.','Deletion journal is copied independently in this drill; durable off-database replication and lag monitoring remain a hosted implementation gate.','All restored auth grants/capabilities are quarantined; real identity and payment reconciliation are required before traffic resumes.','No production data or cloud resources were used.']};
