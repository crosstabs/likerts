...
begin;
do $$ begin
  if not exists(select 1 from pg_roles where rolname='likerts_erasure_archiver' and rolcanlogin and not rolsuper and not rolbypassrls and not rolinherit and not rolcreatedb and not rolcreaterole and not rolreplication) then
  ...
  if exists(select 1 from pg_roles r where r.rolname='likerts_erasure_archiver' and (
    ...
    exists(select 1 from pg_class c join pg_namespace n on n.oid=c.relnamespace where n.nspname='likerts' and c.relowner=r.oid))) then raise exception 'Archiver must not own database, schema, tables or functions'; end if;
end $$;
