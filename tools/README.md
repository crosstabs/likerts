...
The hosted path uses the scoped service token above. Install the current free edition from the public source repository with Rust stable:
...
If you already have a checkout, run only the Cargo command from its root. This installs `likerts` into Cargo's binary directory; ensure that directory is on `PATH`. The old `/downloads/` artifacts predate the free edition and are no longer served by the website. There is no automatic updater.
...
`responses_aggregate` and `responses_analyze` accept optional `collectionId`, `acceptedFrom`, `acceptedTo` and `minimumGroupSize` filters. They read a stable bounded snapshot without returning respondent metadata, free-text answers or exact dates. Question results require at least three responses by default, and distribution cells below the selected threshold are omitted. `responses_analyze` adds descriptive findings and chart-ready bar data; treat those findings as observations, not causal explanations. Narrow the acceptance window if the workspace exceeds 50,000 responses.
