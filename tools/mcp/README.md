...
For an opt-in check through installed, already authenticated Codex and Claude Code CLIs, add `--clients` to the lifecycle command. This preflights account state and makes one bounded model request per authenticated client using only `surveys_list` and a temporary `surveys:read` credential for an empty local workspace. It does not purchase access, alter global client configuration, or run in ordinary CI. Each client has a three-minute timeout; unavailable authentication or approval is reported separately from the account-free protocol test. The optional run exits nonzero unless every selected client returns a verified empty tool result; use `--clients=claude` to retry only Claude after authentication. Client output is summarized without publishing raw logs or credentials.

On 13 September 2026, Codex CLI 0.154.0 completed a verified `surveys_list` call through public `@likerts/mcp@0.1.0` against a temporary scoped PostgreSQL workspace. Claude Code 2.1.201 parsed the configuration but remains unauthenticated, so its real-client call is still pending. See [exact client evidence and the owner login action](../../docs/verification/agent-clients.md). The earlier Codex failure was traced to the test harness leaving stdin open; immediate EOF and reliable timeout tracking now have regression tests.
