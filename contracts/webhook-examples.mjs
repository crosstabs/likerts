...
const endpoint={id,url:'https://hooks.customer.com/likerts',eventTypes:['response.accepted'],enabled:false,revoked:false,keyId,createdAt:'2026-09-09T00:00:00Z'};
...
export const webhookExamples={
 webhook_endpoints_create:entry({idempotencyKey:'webhook-create-001',url:endpoint.url},credential,'Creates disabled and response-only by default. Opt into credits.threshold_reached with eventTypes and usage:read in addition to webhooks:write; subscriptions are immutable. Install the returned signing secret on your receiver, then explicitly enable. An identical idempotency retry reconstructs the original generation.'),
 ...
 webhook_endpoints_update:entry({id,enabled:true},{...endpoint,enabled:true},'Enable only after configuring verification; enabling a credit subscriber also requires usage:read. Use enabled:false to pause, or revoke:true to permanently cancel queued deliveries; an in-flight HTTP request cannot be recalled.'),
