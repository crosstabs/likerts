...
export const webhookExamples={
 webhook_endpoints_create:entry({idempotencyKey:'webhook-create-001',url:endpoint.url},credential,'Creates a disabled response callback. Install the returned signing secret on your receiver, then explicitly enable it. An identical idempotency retry reconstructs the original generation.'),
 ...
 webhook_endpoints_update:entry({id,enabled:true},{...endpoint,enabled:true},'Enable only after configuring signature verification. Use enabled:false to pause, or revoke:true to permanently cancel queued deliveries; an in-flight HTTP request cannot be recalled.'),
