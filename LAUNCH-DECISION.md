...
| Managed operations | Exercise Neon restore/PITR and cross-replica export recovery; deliver an actual alert; validate customer-owned DNS, adversarial callback behavior, enforced egress and expiry cleanup; record complete provider bills and a sustained load result. Compatible Render restoration, isolated failed-candidate rejection, one same-account public HTTPS callback, loopback-address denial, redirect refusal and timeout retry scheduling already pass. |
